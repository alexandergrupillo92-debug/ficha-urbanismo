...
AREAS_DISPONIBLES = [
    ...
    ("frente", "Fachada principal", False),
    ("fachada_posterior", "Fachada posterior", False),
    ("pared_izq", "Fachada lateral izquierda", False),
    ("pared_der", "Fachada lateral derecha", False),
    ("asentamiento", "Asentamiento", False),
]
...
def _linea_opcional_xml(etiqueta, valor):
    """
    Genera una línea 'Etiqueta: valor' dentro del Cuadro 1 SOLO si el
    valor fue proporcionado. Si viene vacío, no se agrega absolutamente
    nada (ni la etiqueta), para no dejar una línea colgando en el informe.
    """
    ...
    return (
        ...
        f'<w:t xml:space="preserve">{_esc(etiqueta)}: </w:t></w:r>'
        f"<w:r><w:t>{_esc(valor)}</w:t></w:r></w:p>"
    )
...
def generar_informe_docx(data, fotos_paths, docx_path):
    ...
    reemplazos = {
        ...
        "{{LINEA_INSPECTOR_XML}}": _linea_opcional_xml("Inspector de Obras", data.get("inspector", "")),
