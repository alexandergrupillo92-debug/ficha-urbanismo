...
def generar_informe_docx(data, fotos_paths, docx_path):
    ...
    reemplazos = {
        ...
        "{{OBS_ESTRUCTURA}}": _esc(data.get("obs_estructura", "")),
        "{{OBS_TECHO}}": _esc(data.get("obs_techo", "")),
        "{{OBS_ELECTRICAS}}": _esc(data.get("obs_electricas", "")),
        "{{OBS_SANITARIAS}}": _esc(data.get("obs_sanitarias", "")),
        "{{OBS_ACABADOS}}": _esc(data.get("obs_acabados", "")),
...
if __name__ == "__main__":
    ...
    data_prueba = {
        ...
        "obs_estructura": "Estructura en buen estado general, sin grietas visibles.",
        "obs_techo": "Techo de platabanda sin filtraciones aparentes.",
        "obs_electricas": "Instalación eléctrica visible, con tablero y breakers en buen estado.",
        "obs_sanitarias": "Instalaciones sanitarias completas y funcionales.",
        "obs_acabados": "Faltan puertas internas en dos habitaciones.",
        "recomendaciones": [
