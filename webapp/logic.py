...
def generar_pdf(sesion, carpeta):
    ...
    filas_tabla = [[Paragraph(h, st_th) for h in ["Grupo", "Unidad", "Propietario / Estatus / Tel.", "Friso", "Techo / Cubierta", "Baños", "Electricidad", "Patología", "Foto"]]]
    ...
    for i, u in enumerate(sesion["unidades"], start=1):
        ...
        filas_tabla.append([
            ...
            celda_foto(u.get("foto"), st_tb),
        ])

    t_unidades = Table(filas_tabla, colWidths=[1.8 * cm, 1.4 * cm, 2.6 * cm, 1.5 * cm, 2.3 * cm, 2.3 * cm, 1.5 * cm, 1.9 * cm, 2.0 * cm])
