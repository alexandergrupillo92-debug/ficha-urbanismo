...
from logic_informe import generar_informe_docx, AREAS_DISPONIBLES
from logic_ficha_familiar import generar_ficha_familiar_docx
...
def _fecha_larga(fecha_iso):
    """Convierte '2026-09-15' (input type=date) a '15 de septiembre de 2026'."""
...
# ============================================================
# INFORME DE INSPECCIÓN DE VIVIENDA (MODELO 1 - CLÁSICO)
# ============================================================
...
def informe_nuevo():
    ...
    f = request.form
    nombre_ocupante = f.get("nombre", "").strip() or "Sin Identificar"

    data = {
        "fecha": _fecha_larga(f["fecha"]) if f.get("fecha") else datetime.now().strftime("%d/%m/%Y"),
        "direccion": f.get("direccion", ""),
        "parroquia": f.get("parroquia", ""),
        "tipo_vivienda": f.get("tipo_vivienda", ""),
        "ambientes": f.get("ambientes", ""),
        "nombre": nombre_ocupante,
        "cedula": f.get("cedula", ""),
        "telefono": f.get("telefono", ""),
        "adultos": f.get("adultos", "0"),
        "menores": f.get("menores", "0"),
        "condicion": f.get("condicion", ""),
        "inspector": f.get("inspector", "").strip(),
        "inspeccion": [],
        "recomendaciones": [
            linea.strip() for linea in f.get("recomendaciones", "").splitlines() if linea.strip()
        ],
        "captions": [f.get(f"caption_{i}") or f"Fig. {i}" for i in range(1, 5)],
    }

    for clave, etiqueta, tiene_cantidad in AREAS_DISPONIBLES:
        if not f.get(f"area_{clave}"):
            continue
        observacion = f.get(f"obs_{clave}", "").strip()
        if not observacion:
            continue
        if tiene_cantidad:
            cantidad = f.get(f"cant_{clave}", "").strip()
            elemento = f"{etiqueta} ({cantidad})" if cantidad else etiqueta
        else:
            elemento = etiqueta
        data["inspeccion"].append({"elemento": elemento, "observacion": observacion})

    if not data["inspeccion"]:
        return render_template("informe_form.html", error="Marca al menos un área con patología en el Cuadro 2.")

    fotos_subidas = request.files.getlist("fotos")
    if len(fotos_subidas) != 4 or any(not fo or not fo.filename for fo in fotos_subidas):
        return render_template("informe_form.html", error="Debes subir exactamente 4 fotos.")

    fotos_paths = [
        guardar_foto(fo, f"{nombre_ocupante}_informe_{i}")
        for i, fo in enumerate(fotos_subidas, start=1)
    ]

    slug = re.sub(r"[^A-Za-z0-9_-]", "_", nombre_ocupante)[:40]
    nombre_archivo = f"Informe_Inspeccion_{slug}_{datetime.now().strftime('%Y%m%d%H%M%S')}.docx"
    ruta = os.path.join(CARPETA, nombre_archivo)
    generar_informe_docx(data, fotos_paths, ruta)

    return render_template("informe_listo.html", nombre_archivo=nombre_archivo)


# ============================================================
# FICHA FAMILIAR DE HÁBITAT Y VIVIENDA (aparte del informe;
# solo texto, sin fotos ni menús desplegables)
# ============================================================
@app.route("/ficha_familiar", methods=["GET", "POST"])
def ficha_familiar():
    if request.method == "GET":
        return render_template("ficha_familiar_form.html")
    ...
    data = {
        ...
        "parroquia": f.get("parroquia", ""),
        "grado_estudio": f.get("grado_estudio", ""),
        "ocupacion_jefe": f.get("ocupacion_jefe", ""),
        "descripcion_caso": f.get("descripcion_caso", ""),
        "nucleo": [],
    }
    for i in range(1, 6):
        data["nucleo"].append({
            "nombre": f.get(f"n{i}_nombre", ""),
            "ci": f.get(f"n{i}_ci", ""),
            "fecha": f.get(f"n{i}_fecha", ""),
            "edad": f.get(f"n{i}_edad", ""),
            "parentesco": f.get(f"n{i}_parentesco", ""),
            "ocupacion": f.get(f"n{i}_ocupacion", ""),
            "escolaridad": f.get(f"n{i}_escolaridad", ""),
            "discapacidad": f.get(f"n{i}_discapacidad", ""),
            "observacion": f.get(f"n{i}_observacion", ""),
        })

    slug = re.sub(r"[^A-Za-z0-9_-]", "_", (data["nombres"] + "_" + data["apellidos"]).strip("_ ") or "Sin_Identificar")[:40]
    nombre_archivo = f"Ficha_Familiar_{slug}_{datetime.now().strftime('%Y%m%d%H%M%S')}.docx"
    ruta = os.path.join(CARPETA, nombre_archivo)
    generar_ficha_familiar_docx(data, ruta)

    return render_template("ficha_familiar_listo.html", nombre_archivo=nombre_archivo)
